...
export default class extends Controller {
  ...
  connect() {
    ...
    this.countrySvgs = {}  // Store SVG for each guessed country
  ...
  setupLayers() {
    ...
    // Guessed countries outline layer - shown only for guessed countries during gameplay
    this.map.addLayer({
  ...
  selectRegion(event) {
    ...
    // Don't show guessed list during gameplay in Easy mode
    ...
    // Easy mode: Show outlines for region countries from the start
    this.map.setFilter("countries-base", ["in", "ADM0_A3"])
    this.map.setFilter("countries-outline", ["in", "ADM0_A3", ...this.regionCountries])

    // Initialize empty correct countries and names layers (filled as player guesses)
    this.map.setFilter("countries-correct", ["in", "ADM0_A3"])
    this.map.setFilter("country-names", ["in", "ADM0_A3"])
    // Don't use the guessed-outline layer in Easy mode (regular outline is always visible)
    this.map.setFilter("countries-guessed-outline", ["in", "ADM0_A3"])
  ...
  updateGuessedList() {
    // For Easy mode, only show unguessed (remaining) countries after finish
    ...
    const remainingCountries = remaining.map(code => ({
      name: countriesMapping[code]?.display_name || code,
      correct: false,
      code: code
    }))
    ...
    this.guessedListTarget.innerHTML = `
      <div class="guessed-grid">
        ${remainingCountries.map(country => {
          let svg = this.countrySvgs[country.code]
          if (!svg) {
            svg = this.extractCountrySvg(country.code)
            if (svg) {
              this.countrySvgs[country.code] = svg
            }
          }

          return `<div class="guessed-country incorrect">
  ...
  submitGuess() {
    ...
    if (this.regionCountries.includes(countryCode)) {
      ...
      // Update map to fill this country in green (Easy mode - outline already visible)
    ...
    this.hideDropdown()
    this.updateStats()
    // Don't update guessed list during gameplay in Normal mode
