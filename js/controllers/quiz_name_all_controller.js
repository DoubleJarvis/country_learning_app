    ...
    this.guessedListTarget.innerHTML = `
      <div class="guessed-grid">
        ${allGuesses.map(guess => {
          let svg = this.countrySvgs[guess.code]
          if (!svg) {
            svg = this.extractCountrySvg(guess.code)
            if (svg) {
              this.countrySvgs[guess.code] = svg
            }
          }

          return `<div class="guessed-country ${guess.correct ? 'correct' : 'incorrect'}">
