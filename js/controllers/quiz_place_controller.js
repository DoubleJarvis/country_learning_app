...
export default class extends Controller {
  ...
  nextCountry() {
    ...
    let svg = this.countrySvgs[this.currentCountry]
    if (!svg) {
      svg = this.extractCountrySvg(this.currentCountry)
      if (svg) {
        this.countrySvgs[this.currentCountry] = svg
      }
    }
    this.shapeTarget.innerHTML = svg || `<div class="place-shape-fallback">?</div>`
    this.resizeShape()
  }
